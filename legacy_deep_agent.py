"""
Deep Agent with E2B Sandbox Integration (LEGACY - deepagents version)
...
NOTE: This is the legacy deepagents implementation. The project has migrated to LangGraph.
See deep_agent.py for the new implementation.
"""
...
class LegacyDeepAgentE2B:
    ...
    LEGACY: This uses the old deepagents framework. See DeepAgentE2B in deep_agent.py for the new version.
    """
...
Remember: You're operating in a secure E2B sandbox, so you can safely execute code, install packages, and experiment without affecting the host system."""
    ...
    def invoke(self, task: str) -> dict:
        ...
        response = asyncio.run(self.agent.ainvoke({"messages": [{"role": "user", "content": task}]}))
